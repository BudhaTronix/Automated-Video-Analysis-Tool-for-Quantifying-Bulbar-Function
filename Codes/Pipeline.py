...
class pipeline:
    ...
    def callStabilize(self, MainFile, SMOOTHING_RADIUS):
    ...
    def callCompress(self, MainFile):
    ...
    def callFaceDetect(self, MainFile, correctionFactor_Face):
    ...
    def callLipExtraction(self, MainFile, correctionFactor_Lip):
    ...
    def callTongueTrack(self, MainFile, threshold, thresh_iterations, disp, visual_area, time_slice, model,
                        save_in_excel):
        ...
        if save_in_excel:
            ...
            self.excelUpadter(timestamp, Filename, time_slice, frequency_total, errors, sweeps_mode, time_mode,
                              sweeps_mean
                              , time_mean, stdev)
    ...
    def CUI(self, MainFile, Stabilize, Video_Compression,
        ...
        print("\n")
        if Stabilize:
            self.callStabilize(MainFile, SMOOTHING_RADIUS)

        # Call Video Compression
        if Video_Compression:
            self.callCompress(MainFile)

        # Call Face Extraction
        if Face_Extract:
            self.callFaceDetect(MainFile, correctionFactor_Face)

        # Call Lip Extraction
        if Lip_Extraction:
            self.callLipExtraction(MainFile, correctionFactor_Lip)

        # Call Frequency Calculation
        if Frquency_Calculation:
            self.callTongueTrack(MainFile, threshold, thresh_iterations, disp, visual_area, time_slice, model,
                                 save_in_excel)
